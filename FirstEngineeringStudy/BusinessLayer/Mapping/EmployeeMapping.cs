...
namespace FirstEngineeringStudy.BusinessLayer.Mapping
{
    ...
    {
        public EmployeeResponseVm DataModelToRetrievalViewModel(
            Employee employee)
        {
            if (employee == null)
                throw new ArgumentException("Employee must not be null.");

            return new EmployeeResponseVm(employee.EmployeeId, employee.Email,
                employee.EmployeeNumber,
                employee.FullName, employee.DOB, employee.StartDate,
                employee.Salary);
        }
        ...
        {
            return employees
                .Select(employee => DataModelToRetrievalViewModel(employee))
                .ToArray();
        }
    }
